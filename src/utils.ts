 ...
 */
export async function validateClaudeProject(projectRoot: string): Promise<boolean> {
  try {
 ...
 */
export async function ensureVisualizerDir(vizDir: string): Promise<void> {
...
export async function findAvailablePort(preferred: number): Promise<number> {
  ...
  throw new Error(
    `No available port found near ${preferred}. ` +
